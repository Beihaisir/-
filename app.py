...
# ========== 静态内置字体（优先使用 STFANGSO.TTF）==========
def get_font_path():
    custom_font = os.path.join("static", "fonts", "STFANGSO.TTF")
    ...
    st.warning("未找到中文字体文件。请将 STFANGSO.TTF 放到 static/fonts/ 目录下，否则PDF导出将无法显示中文。")
    return None
...
def create_pdf(questions: List[Dict], title: str, font_path: str):
    if not font_path or not os.path.exists(font_path):
        st.error("字体文件缺失，无法生成PDF。请将 STFANGSO.TTF 放入 static/fonts/ 目录。")
        return None
    try:
        pdf = PDF(font_path)
        pdf.add_page()
...
def main():
    ...
    if font_path:
        st.success("已加载中文字体，PDF可正常导出。")
    else:
        st.info("未找到中文字体，PDF导出功能将不可用。请将 STFANGSO.TTF 放入 static/fonts/ 目录。")
